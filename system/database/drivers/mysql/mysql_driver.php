...
//---change---//
		...
		if( $this->is_write_type($sql) ){
          $sql = $this->_prep_query($sql);
	      if( $this->conn_id->run_sql($sql) ){
	        return array( array( 'last_id'=> $this->conn_id->last_id() ) );
	      }else{
	      	return false;
	      }
		}else{
			$sql = $this->_prep_query($sql);
		    $result = $this->conn_id->get_data($sql);
		}
		
		if( $this->conn_id->errno() != 0){
		  return false;
		}
...
*/	
		return $this->conn_id->affectedRows();
	}
	// --------------------------------------------------------------------
