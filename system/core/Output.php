...
class CI_Output {
	...
	{
		$CI =& get_instance();	
		$cache_domain = $CI->config->item('cache_path');
		$cache_method = $CI->config->item('cache_method');
...
//	if ( $cache_domain == ''){	return FALSE;}
    ...
    if( $cache_method == "storage") {
			...
			$storage->write( $cache_domain , $cache_file_name , $expire.'TS--->'.$output );
		} else if( $cache_method == "memcache" ){
			$mmc=memcache_init();
			if( $mmc != FALSE){
			   memcache_set($mmc,$cache_file_name,$expire.'TS--->'.$output);
		  }
		}
		log_message('debug', "Cache file written: ".$cache_file_name);
